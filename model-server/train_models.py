#!/usr/bin/env python3
"""Train PCA + Logistic Regression and PCA + ElasticNet models per threat type."""
import pandas as pd
import numpy as np
import pickle
...
def train_model_for_threat_type(threat_df, threat_name, max_components=26, 
    ...
    numeric_cols = [col for col in threat_df.columns 
                   if col not in exclude_cols and threat_df[col].dtype in ['int64', 'float64', 'float32']]
...
def train_linear_regr_model_pca(df, threat_name=None, max_components=26, 
                            val_fraction=0.2, random_state=42, target_col='Financial_Loss_MUSD'):
    """
    Train a Linear Regression model with PCA for a specific threat type.
    ...
    Args:
        ...
        val_fraction: Fraction of data to use for validation
        random_state: Random seed for reproducibility
        target_col: Column name of the target variable (e.g., 'Financial_Loss_MUSD' or 'actual_days_to_stabilization')
    ...
    print(f"\n{'='*70}")
    if threat_name:
        print(f"Training Linear Regression model for {threat_name.upper()} threat type")
        df = df[df["Threat Type"] == threat_name].copy()
    print(f"{'='*70}")
    ...
    model_package = {
        ...
        'median_values': None, # not needed for regression since we dropped rows with missing values
        'validation_metric': best_mse,
        'cumulative_variance_explained': cumulative_variance
    }
    return model_package 
...
def main():
    ...
    data_path = "../Data/unh_hackathon_prompt_2_data.json" # Data/unh_hackathon_prompt_2_data.json
    ...
    # Train a model for each threat type and output variable (3 outputs x 5 threat types = 15 models)
    models = {}
    for threat_name, threat_df in threat_dfs.items():
        model_package0 = train_model_for_threat_type(
            threat_df, 
            threat_name, 
            max_components=26,
            val_fraction=0.2,
        ...
        model_package1 = train_linear_regr_model_pca(
            threat_df,
            threat_name,
            max_components=26,
            val_fraction=0.2,
            random_state=42,
            target_col='Financial_Loss_MUSD'
        )

        model_package2 = train_linear_regr_model_pca(
            threat_df,
            threat_name,
            max_components=26,
            val_fraction=0.2,
            random_state=42,
            target_col='actual_days_to_stabilization'
        )

        models[f"{threat_name}_response_success"] = model_package0
        models[f"{threat_name}_financial_loss"] = model_package1
        models[f"{threat_name}_neutralization"] = model_package2
    ...
    for model_name, model_package in models.items():
    ...
    summary = {
        ...
        'model_info': {
            model_name: {
                ...
                'validation_metric': pkg['validation_metric'],
                'variance_explained': pkg['cumulative_variance_explained']
            }
            for model_name, pkg in models.items()
        }
    }
    ...
    # Print summary table
    print("\nModel Summary:")
    print(f"{'Model Name':<20} {'Components':<12} {'Val Accuracy':<15} {'Variance Explained':<20}")
    print("-" * 70)
    for model_name in summary['model_info'].keys():
        info = summary['model_info'][model_name]
        print(f"{model_name:<20} {info['n_components']:<12} {info['validation_metric']:.4f}         {info['variance_explained']:.2f}%")
